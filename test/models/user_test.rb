...
class UserTest < ActiveSupport::TestCase
  ...
  test "email validation should accept valid addresses" do
    valid_adresses = %w[user@example.com USER@foo.com A_US_ER@foo.bar.org
      first.last@foo.jp alice+bob@ba.cn]
  ...
  test "password should be present(nonblank) and have a minimum length" do
    @user.password = @user.password_confirmation = "a" * 5
    assert_not @user.valid?
  end

  test "autuehnticated? should return false for a user with nil digest" do
